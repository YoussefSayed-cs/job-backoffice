        ...
        <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-8">
            <form action="{{ route('job-vacancies.store') }}" method="POST">
                ...
                <div class="mb-8">
                    ...
                    <p class="text-sm text-gray-600 mt-1">Fill in the job vacancy information below</p>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="categoryID" id="categoryID"
                        ...
                        @foreach ($jobCategories as $jobCategory)
                            <option value="{{ $jobCategory->id }}" {{ old('categoryID') == $jobCategory->id ? 'selected' : '' }}>
                                {{ $jobCategory->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-8">
                    ...
                    <textarea rows="5" name="description" id="description"
                        placeholder="Enter a detailed description of the job vacancy..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition resize-none {{ $errors->has('description') ? 'border-red-500 focus:ring-red-500' : '' }}">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
